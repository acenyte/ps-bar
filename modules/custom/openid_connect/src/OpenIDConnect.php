...
class OpenIDConnect {
  ...
  public function completeAuthorization(OpenIDConnectClientInterface $client, array $tokens, &$destination) {
    ...
    // Ensure, arrays are passed on for extracting and manipulating
    // user information.
    if (empty($user_data)) {
      $user_data = [];
    }
    ...
    if (empty($user_data) && empty($userinfo)) {
      $message = 'No user information provided by @provider';
      $variables = ['@provider' => $client->getPluginId()];
      $this->logger->error($message . ' (@code @error). Details: @details', $variables);
      return FALSE;
    }

    // Ensure an array is passed on for further extraction of user information.
    if (empty($userinfo)) {
      $userinfo = [];
    }
    // Whether an email address is missing within the userinfo.
